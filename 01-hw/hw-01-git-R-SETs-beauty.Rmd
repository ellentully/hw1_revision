...
```
```{r attempt 1, eval=TRUE}
beauty %>% 
  group_by(female,age) %>%
  tally(sort = TRUE) %>%
  arrange(-female, n, age)
```

4.  Based on the previous output we can easily identify the most common sex at each professor age, but the output is sorted by `n` (the frequencies) as opposed to being organized by the `female`. Build on the pipeline to arrange the results so that they're arranged by `female` first, and then `n`. This means you will need to add one more step to the pipeline, and you have two options: `arrange(age, n)` or `arrange(n, age)`. You should try both and decide which one organizes the output by species and then ranks the names in order of frequency for each species.
